...
export default function TokenSense() {
  ...
  const pay = () => {
    ...
    const params = new URLSearchParams({
      ...
      fiatAmount: String(val),
      fiatCurrency: "USD",
      defaultPaymentMethod: "apple_pay",
      themeColor: "000000",
    });
    const url = `https://buy.transak.com/?cryptoCurrencyCode=USDT&networks=polygon&walletAddress=${WALLET}&fiatAmount=${val}&fiatCurrency=USD&defaultPaymentMethod=apple_pay`;
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20, justifyContent: "center" }}>
          {[19, 49, 79, 99].map(v => (
            <button key={v} onClick={() => setAmount(String(v))} style={{
              ...
              padding: "8px 0",
              borderRadius: 8,
              border: `1.5px solid ${amount == v ? "#38bdf8" : "#1e3a5f"}`,
              background: amount == v ? "#0c1e2e" : "transparent",
              color: amount == v ? "#38bdf8" : "#8ba3bc",
              fontSize: 13,
              fontWeight: 700,
              cursor: "pointer",
              fontFamily: "system-ui, sans-serif",
        ...
        <button
          ...
          style={{
            ...
            fontSize: 16,
            fontWeight: 800,
            cursor: !amount || parseFloat(amount) < 1 ? "not-allowed" : "pointer",
            fontFamily: "system-ui, sans-serif",
            transition: "all 0.15s",
            boxShadow: !amount || parseFloat(amount) < 1 ? "none" : "0 4px 24px rgba(3,105,161,0.4)",
          }}
        >
          {amount && parseFloat(amount) >= 1 ? `Pay $${amount}` : "Enter amount"}
        </button>
